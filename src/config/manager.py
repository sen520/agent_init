...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info(f"版本: {config.get('version', '未知')}")
        logger.info("")
        logger.info("配置项:")
        logger.info(f"  最大迭代次数: {get_max_iterations()}")
        logger.info(f"  最大行长度: {get_max_line_length()}")
        logger.info(f"  最大函数长度: {get_max_function_length()}")
        logger.info(f"  备份目录: {get_backup_dir()}")
        logger.info(f"  最大分析文件数: {get_max_files_to_analyze()}")
        logger.info(f"  启用策略: {', '.join(get_enabled_strategies())}")
        logger.info("")
        logger.info("✅ 配置验证通过！")
